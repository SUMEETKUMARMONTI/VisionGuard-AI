...
def draw_count_panel(frame, person_count: int, display_fps: float, inference_fps: float) -> None:
    ...
    # infer_text = f"Inference: {inference_fps:.1f} fps"
    # (iw, _), _ = cv2.getTextSize(infer_text, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1)
    # cv2.putText(frame, infer_text, (fw // 2 - iw // 2, 33),
...
def main():
    ...
    try:
        while True:
            ...
            draw_count_panel(frame, person_count, fps_display, inference_fps)
